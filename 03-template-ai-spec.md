...
Artifact `eval/golden_set.*` chưa có trong repo tại thời điểm cập nhật spec; phải được tạo trước CP3 để rubric có thể kiểm lại.
...
Chưa được xem là một lượt golden-set đầy đủ. Chưa đủ dữ liệu để tuyên bố đạt quality bar 85%.
...
| Hạng mục | Owner có bằng chứng trong repo | Trạng thái |
|---|---|---|
| Agent, ChromaDB, Gemini OCR/embedding, backend | duyvu1105 | Đã build |
| Frontend hai vai trò và teacher queue | duyvu1105 | Đã build |
| Spec | Cần nhóm xác nhận tên owner trước nộp | Bản này đã hoàn thiện nội dung kỹ thuật/evidence |
| Evidence mining | Cần nhóm xác nhận | Đã có phương pháp và số liệu |
| Golden set/evaluation | Cần phân công | Chưa có artifact `eval/` |
| User validation/demo | Cần phân công | Chưa có artifact `validation/` |
...
1. Điền Zone và danh sách đầy đủ thành viên/phân công có tên.
2. Tạo `eval/golden_set` ≥20 case và chạy trọn bộ ít nhất một lượt.
3. Thu thập ≥3 willing users có tên và ≥5 feedback từ ≥5 người ngoài nhóm.
4. Ghi feedback vào `validation/` và cập nhật changelog theo feedback.
5. Bổ sung worksheet JTBD/Canvas CP1 và link artifact evidence mining có script chạy lại.
